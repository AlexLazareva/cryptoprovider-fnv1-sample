# Создание плагина криптопровайдера для веб-версии Pilot-ICE Enterprise

## Введение

Из коробки веб-версия Pilot-ICE Enterprise работает с КриптоПро ЭЦП Browser plug-in, который использует формат электронной подписи CADES-BES. Однако если нужно поддержать какой-то альтернативный формат, то для таких случаев Pilot-Web предоставляет возможность создания собственных плагинов криптопровайдеров через открытый SDK.
...
`tsconfig.json` с базовыми настройками:
...
Webpack используется для создания модуля, который можно динамически загрузить в веб-версию Pilot-ICE через Расширения. Ключевой момент — использование **Module Federation Plugin**.
...
`src/assets/extensions.config.json`:
...
Этот файл описывает метаданные плагина и указывает веб-версии Pilot-ICE, какой интерфейс реализует модуль.

## Главный класс и вспомогательные файлы
